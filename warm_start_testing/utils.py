...
###############################################################################
###############################################################################
def feature_selection_info_query(feature_name=None, corr_threshold=0.98):
    """Checking the corrlection results."""
    file_processor = LoadSave()
    res = file_processor.load_data(".//submission_logs//feature_selection.pkl")
    train_importances, train_corr = res[0], res[1]
    if feature_name not in train_importances["feature_name"].values:
        return None

    # Basic information
    target_record = train_importances[train_importances["feature_name"] == feature_name]
    rank, info = str(list(target_record.index)[0]) + "/" + str(len(train_importances)), target_record["mutual_info"].values[0]

    # Basic corr infomation
    train_corr = train_corr[train_corr[feature_name] > corr_threshold][["feature_name", feature_name]]
    train_corr = train_corr.sort_values(by=feature_name, ascending=False).iloc[1:]
    return rank, info, train_corr
